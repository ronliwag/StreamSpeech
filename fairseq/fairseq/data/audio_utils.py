...
def convert_waveform(
    ...
    # Windows-compatible audio processing without SOX
    try:
        effects = []
        if normalize_volume:
        ...
        if "sox extension is not supported on Windows" in str(e):
            # Fallback: Use torchaudio's resample function for Windows
            import torchaudio
            print("Using Windows-compatible audio processing in convert_waveform (no SOX)")
            
            is_np_input = isinstance(waveform, np.ndarray)
            _waveform = torch.from_numpy(waveform) if is_np_input else waveform
            
            # Handle mono conversion and ensure correct tensor shape
            if to_mono and _waveform.shape[0] > 1:
                _waveform = torch.mean(_waveform, dim=0, keepdim=True)
            elif to_mono and len(_waveform.shape) == 1:
                # Ensure 2D tensor shape (1 channel x samples)
                _waveform = _waveform.unsqueeze(0)
            
            # Handle resampling
            if to_sample_rate is not None and to_sample_rate != sample_rate:
                resampler = torchaudio.transforms.Resample(sample_rate, to_sample_rate)
                _waveform = resampler(_waveform)
                converted_sample_rate = to_sample_rate
            else:
                converted_sample_rate = sample_rate
            
            # Handle volume normalization (basic implementation)
            if normalize_volume:
                _waveform = _waveform / torch.max(torch.abs(_waveform))
            
            if is_np_input:
                _waveform = _waveform.numpy()
            
            return _waveform, converted_sample_rate
        else:
            raise
...
def _get_torchaudio_fbank(
    ...
    try:
        ...
        # Ensure 2D tensor shape (channels x samples) for fbank
...
def parse_path(path: str) -> Tuple[str, List[int]]:
    ...
    if Path(path).suffix in FEATURE_OR_SF_AUDIO_FILE_EXTENSIONS:
        ...
        if len(path) >= 2 and path[1] == ':' and len(path.split(':')) > 2:
            # Windows path with drive letter and additional colons (slice info)
            parts = path.split(':')
            _path = parts[0] + ':' + parts[1]  # Drive letter + rest of path
            slice_ptr = parts[2:]  # Remaining parts are slice info
        elif len(path) >= 2 and path[1] == ':' and len(path.split(':')) == 2:
            # Windows path with drive letter but no slice info
            _path = path
            slice_ptr = []
        else:
